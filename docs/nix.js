const Nix = {
  ...
  methods: {
    ...
    async saveImage() {
      logDebug("Nix", "saveImage()");

      // const div = document.getElementById("toBeCaptured");
      // logDebug("Nix", "saveImage() div: " + JSON.stringify(div));
      // console.table(div);
      ...
      /*
      function getScreenshotOfElement(element, posX, posY, width, height, callback) {
        // logDebug("Nix", "getScreenshotOfElement() element: " + JSON.stringify(element));
        ...
        html2canvas(element, {
          ...
          idata.data.set(imageData);
          outputContext.putImageData(idata, 0, 0);
          logDebug("Nix", "getScreenshotOfElement() idata: " + JSON.stringify(idata));
      ...
      getScreenshotOfElement(document.querySelector("#toBeCaptured"), 0, 0, 1000, 1000, async function(data) {
        ...
        // exmaple for displaying the image in an <img>
        // $("img#captured").attr("src", "data:image/png;base64,"+data);
        logDebug("Nix", "saveImage() data: " + JSON.stringify(data));
        const blob = await fetch(data).then(r => r.blob());
        logDebug("Nix", "saveImage() blob: " + JSON.stringify(blob));
        const url = URL.createObjectURL(blob);
        logDebug("Nix", "saveImage() url: " + JSON.stringify(url));
      ...
      */
      ...
      // html2canvas(element).then(async (canvas) => {
        ...
        var context = canvas.getContext('2d');
        var imageData = context.getImageData(posX, posY, width, height).data;
        logDebug("Nix", "saveImage() imageData: " + JSON.stringify(imageData));
        var outputCanvas = document.createElement('canvas');
        var outputContext = outputCanvas.getContext('2d');
        outputCanvas.width = width;
        outputCanvas.height = height;
        ...
        logDebug("Nix", "saveImage() dataURL: " + JSON.stringify(dataURL));
    ...
    setCanvasSize() {
      // logDebug("Nix", "setCanvasSize() canvasSetting: " + JSON.stringify(this.canvasSetting));
      this.canvas.setWidth(this.canvasSetting.width);
      this.canvas.setHeight(this.canvasSetting.height);
    },
    setFrame() {
      logDebug("Nix", "setFrame() this.gif.frame: " + JSON.stringify(this.gif.frame));
    ...
    onFileChange(file) {
      ...
      // logDebug("Nix", "onFileChange() url: " + JSON.stringify(url));
      var imgObj = new Image();
      imgObj.src = url;
      // logDebug("Nix", "onFileChange() imgObj: " + JSON.stringify(imgObj));
      imgObj.onload = function () {
        const image = new fabric.Image(imgObj);
        image.set({ left: 0, top: 0, angle: 0, padding: 0, cornersize: 0 });
        // logDebug("Nix", "onFileChange() image width: " + image.width + ", height: " + image.height);
        // logDebug("Nix", "onFileChange() t.canvas width: " + t.canvas.width + ", height: " + t.canvas.height);
    ...
    async onGIFFrameSelectionModalOpened() {
      logDebug("Nix", "onGIFFrameSelectionModalOpened()");
      const t = this;
    ...
    async addGIFFrame() {
      const t = this;
      logDebug("Nix", "addGIFFrame() t.gif.rub: " + JSON.stringify(t.gif.rub, null, 2));
      logDebug("Nix", "addGIFFrame() t.gif.rub.get_canvas(): " + JSON.stringify(t.gif.rub.get_canvas(), null, 2));
      const scale = 0.5;
      const x = t.gif.rub;
      const y = x.get_canvas().getContext("2d");
      const z = x.get_canvas().toDataURL();
      logDebug("Nix", "addGIFFrame() z: " + JSON.stringify(z, null, 2));
      fabric.Image.fromURL(z, function(oImg) {
        oImg.set('imageSmoothing', false).scale(scale);
        // logDebug("Nix", "addAsset() adding: " + JSON.stringify(oImg));
        t.canvas.add(oImg);
        logDebug("Nix", "addGIFFrame() added: " + JSON.stringify(oImg));
        logDebug("Nix", "addGIFFrame() LocalStorage.setItem: " + JSON.stringify(oImg));
        localStorage.setItem('canvas', JSON.stringify(t.canvas));
      } , {crossOrigin: 'anonymous'});
      // var element = document.getElementById("thegif1")
    },
    async addAsset(asset) {
      logDebug("Nix", "addAsset() asset: " + JSON.stringify(asset, null, 2));
      const t = this;
      var element = document.getElementById("thegif1")
      ...
      xhr.onload = function() {
        ...
        if (xhr.response.substring(0, 3) == "GIF") {
          logDebug("Nix", "addAsset() asset GIF: " + JSON.stringify(asset, null, 2));
          ...
          fabric.Image.fromURL(asset.image_url, function(oImg) {
            ...
            // logDebug("Nix", "addAsset() adding: " + JSON.stringify(oImg));
            t.canvas.add(oImg);
            logDebug("Nix", "addAsset() added: " + JSON.stringify(oImg));
            logDebug("Nix", "addAsset() LocalStorage.setItem: " + JSON.stringify(oImg));
            localStorage.setItem('canvas', JSON.stringify(t.canvas));
          } , {crossOrigin: 'anonymous'});
        }
      };
    ...
    async addAssetOld(asset) {
      ...
      logDebug("Nix", "EXIF.getData getting 1: " + JSON.stringify(asset.image_url));
      // this.img.src = asset.image_url;
      // logDebug("Nix", "EXIF.getData getting 2: " + JSON.stringify(asset.image_url));
      // var element = document.getElementById("thegif")
      // logDebug("Nix", "EXIF.getData getting 3: " + JSON.stringify(asset.image_url));
      // EXIF.getData(new File(asset.image_url), function() {
      //   logDebug("Nix", "EXIF.getData this: " + JSON.stringify(this));
    ...
    async addImage(nftType, id, image, asset) {
      logDebug("Nix", "addImage() type: " + nftType + ", id: " + id + ", image: " + image);
      if (asset != null) {
        // console.table(asset);
      ...
      fabric.Image.fromURL(image, function(oImg) {
        ...
        // oImg.filters.push(new fabric.Image.filters.Grayscale());
        // oImg.applyFilters();
        logDebug("Nix", "addImage() adding: " + JSON.stringify(oImg));
        t.canvas.add(oImg);
        logDebug("Nix", "addImage() added: " + JSON.stringify(oImg));
      } , {crossOrigin: 'anonymous'});
    },

    async addText(text) {
      logDebug("Nix", "addText() text: " + JSON.stringify(text, null, 2));
    ...
    async loadNFTs() {
      ...
      await delay(DELAY);
      logDebug("Nix", "loadNFTs() 1 - this.accounts: " + this.accounts);
      for (let accountIndex in this.accounts) {
        logDebug("Nix", "loadNFTs() 2");
        const account = this.accounts[accountIndex];
        logDebug("Nix", "loadNFTs() 2 - account: " + account);
        let completed = false;
        let page = 0;
        while (!completed) {
          if (account != null) {
            ...
            logDebug("Nix", "loadAssets() url:" + url);
            const data = await fetch(url).then(response => response.json());
            if (data.assets && data.assets.length > 0) {
              for (let assetIndex = 0; assetIndex < data.assets.length; assetIndex++) {
                const asset = data.assets[assetIndex];
                // logDebug("Nix", "loadAssets() asset(" + (parseInt(offset) + assetIndex) + ") name: " + asset.collection.name + ", slug: " + asset.collection.slug);
  ...
  mounted() {
    ...
    function removeObjectBackground(eventData, transform) {
      if (transform.target.type == "image") {
        logDebug("Nix", "Calling removeObjectBackground()  target: " + JSON.stringify(transform.target));
        ...
        logDebug("Nix", "Calling removeObjectBackground() data: " + JSON.stringify(data));
        if (data.data[3] != 0) {
...
// 648196
    ...
    // logDebug("Nix", "Calling fabric.Object.prototype.controls: " + JSON.stringify(fabric.Object.prototype.controls));
    ...
    this.canvas.on({
      ...
      'object:modified': function(e) {
        ...
        //     logDebug("Nix", "Canvas object:modified(): " + JSON.stringify(objects[i]));
        // }
        logDebug("Nix", "canvas object:modified: " + JSON.stringify(t.canvas));
        localStorage.setItem('canvas', JSON.stringify(t.canvas));
      },
      'selection:cleared': function(e) {
        // var objects = t.canvas.getObjects();
        // for (let i in objects) {
        //     logDebug("Nix", "Canvas selection:cleared(): " + JSON.stringify(objects[i]));
        // }
      },
      'selection:updated': function(e) {
        // var objects = t.canvas.getObjects();
        // for (let i in objects) {
        //     logDebug("Nix", "Canvas selection:updated(): " + JSON.stringify(objects[i]));
        // }
      },
      'selection:created': function(e) {
        // logDebug("Nix", "Canvas selection:created() e: " + JSON.stringify(e));
        // // t.selectedObject = e;
        // var objects = t.canvas.getObjects();
        // for (let i in objects) {
        //     logDebug("Nix", "Canvas selection:created(): " + JSON.stringify(objects[i]));
        // }
      },
      'mouse:down': function(options) {
        if (options.target) {
          // logDebug("Nix", "Canvas mouse:down(): " + JSON.stringify(options.target));
          t.selectedObject = options.target;
          // var objects = t.canvas.getObjects();
          // for (let i in objects) {
          //     logDebug("Nix", "Canvas mouse:down(): " + JSON.stringify(objects[i]));
          // }
        }
      }
    });
