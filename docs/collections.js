const Collections = {
  template: `
    <div class="mt-5 pt-3">
      <b-card no-body header="Collections" class="border-0" header-class="p-1">
...
{{ JSON.stringify(selectedObject, null, 4) }}
  ...
  methods: {
    ...
    async saveImage() {
      logDebug("Collections", "saveImage()");

      // const div = document.getElementById("toBeCaptured");
      // logDebug("Collections", "saveImage() div: " + JSON.stringify(div));
      // console.table(div);
      ...
      /*
      function getScreenshotOfElement(element, posX, posY, width, height, callback) {
        // logDebug("Collections", "getScreenshotOfElement() element: " + JSON.stringify(element));
        ...
        html2canvas(element, {
          ...
          idata.data.set(imageData);
          outputContext.putImageData(idata, 0, 0);
          logDebug("Collections", "getScreenshotOfElement() idata: " + JSON.stringify(idata));
      ...
      getScreenshotOfElement(document.querySelector("#toBeCaptured"), 0, 0, 1000, 1000, async function(data) {
        ...
        // exmaple for displaying the image in an <img>
        // $("img#captured").attr("src", "data:image/png;base64,"+data);
        logDebug("Collections", "saveImage() data: " + JSON.stringify(data));
        const blob = await fetch(data).then(r => r.blob());
        logDebug("Collections", "saveImage() blob: " + JSON.stringify(blob));
        const url = URL.createObjectURL(blob);
        logDebug("Collections", "saveImage() url: " + JSON.stringify(url));
      ...
      */
      ...
      // html2canvas(element).then(async (canvas) => {
        ...
        var context = canvas.getContext('2d');
        var imageData = context.getImageData(posX, posY, width, height).data;
        logDebug("Collections", "saveImage() imageData: " + JSON.stringify(imageData));
        var outputCanvas = document.createElement('canvas');
        var outputContext = outputCanvas.getContext('2d');
        outputCanvas.width = width;
        outputCanvas.height = height;
        ...
        logDebug("Collections", "saveImage() dataURL: " + JSON.stringify(dataURL));
